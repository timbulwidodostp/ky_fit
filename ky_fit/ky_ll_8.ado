...
** NOTE: I changed the programming in this version to make it easier to follow 
** and have less errors due to typos. 
...
/*
 ...
 r_i
 r1=e_i
 r2=e_i+rho_s(e_i-mu_e)+v_i
 r3=t_i
 
 corr(w_i,e_i)!=0
*/

program define ky_ll_8
version 15

** all variables that require estimation
args lnf mu_e     mu_n     mu_w     mu_t     mu_v ///
         ln_sig_e ln_sig_n ln_sig_w ln_sig_t ln_sig_v ///
		 arho_s arho_r arho_w ///
		 lpi_r lpi_s  lpi_w lpi_v
qui {
	*set trace on
	** Program interesting 
	...
	***
	...
	gen double `pi_7'=`pi_r3'*`pi_s1'
	gen double `pi_8'=`pi_r3'*`pi_s2'
	gen double `pi_9'=`pi_r3'*`pi_s3'
	...
	** corr
	...
	gen double `cr_r1s3'= ((1+`rho_s')            *`sig_e'^2+`rho_w'            *`sig_e'*`sig_w')/(`sr_1'*`ss_3')
	gen double `cr_r2s1'= (            (1+`rho_r')*`sig_e'^2                                    )/(`sr_2'*`ss_1')
	gen double `cr_r2s2'= ((1+`rho_s')*(1+`rho_r')*`sig_e'^2                                    )/(`sr_2'*`ss_2')
	gen double `cr_r2s3'= ((1+`rho_s')*(1+`rho_r')*`sig_e'^2+`rho_w'*(1+`rho_r')*`sig_e'*`sig_w')/(`sr_2'*`ss_3')
	local       cr_r3s1 =0
	local       cr_r3s2 =0
	local       cr_r3s3 =0
	...
	** all densities
	...
	replace `rs1'= ($ML_y1-`mean_r')/`sigma_r' 
	replace `rs2'= ($ML_y2-`mean_s')/`sigma_s'
	gen double `lnf7'=-ln(2*_pi*`sigma_r'*`sigma_s'*sqrt(1-`rho_rs'^2))  ///
	                  -1/(2*(1-`rho_rs'^2)) * (`rs1'^2+`rs2'^2-2*`rho_rs'*`rs1'*`rs2') 
	...
	replace `rs1'= ($ML_y1-`mean_r')/`sigma_r' 
	replace `rs2'= ($ML_y2-`mean_s')/`sigma_s'
	gen double `lnf8'=-ln(2*_pi*`sigma_r'*`sigma_s'*sqrt(1-`rho_rs'^2))  ///
	                  -1/(2*(1-`rho_rs'^2)) * (`rs1'^2+`rs2'^2-2*`rho_rs'*`rs1'*`rs2') 
	...
	local sigma_r `sr_3'
	local sigma_s `ss_3'
	local rho_rs  `cr_r3s3'
	
	replace `rs1'= ($ML_y1-`mean_r')/`sigma_r' 
	replace `rs2'= ($ML_y2-`mean_s')/`sigma_s'
	
	gen double `lnf9'=-ln(2*_pi*`sigma_r'*`sigma_s'*sqrt(1-`rho_rs'^2))  ///
	                  -1/(2*(1-`rho_rs'^2)) * (`rs1'^2+`rs2'^2-2*`rho_rs'*`rs1'*`rs2') 
	** all together
	...
	replace `lnf'=log(                   `pi_2'*exp(`lnf2')+`pi_3'*exp(`lnf3') ///
	                 +`pi_4'*exp(`lnf4')+`pi_5'*exp(`lnf5')+`pi_6'*exp(`lnf6') ///
					 +`pi_7'*exp(`lnf7')+`pi_8'*exp(`lnf8')+`pi_9'*exp(`lnf9') ) if $ML_y3==0
    }
 end		 
